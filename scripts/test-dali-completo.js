// Teste completo — Dali da Roça (v2 — schemas corretos)
...
async function api(path, options = {}) {
  ...
  if (!res.ok) {
    const msg = (data.error || data.message || text).substring(0, 100);
...
async function main() {
  console.log('╔══════════════════════════════════════════════════════════╗');
  console.log('║   TESTE COMPLETO — DALI DA ROÇA (v2)                     ║');
  console.log('╚══════════════════════════════════════════════════════════╝\n');

  // ====================================================================
  // 1. LOGIN + SWITCH TENANT
  // ====================================================================
  console.log('━━━ 1. LOGIN + SWITCH ━━━');
  ...
  tenantId = dali.id;
  console.log('   Loja:', dali.companyName, '| ID:', tenantId);

  const sw = await api('/auth/switch-tenant', {
  ...
  // ====================================================================
  // 2. CRIAR USUÁRIOS (Vendedor + Admin loja)
  // ====================================================================
  console.log('\n━━━ 2. CRIAR USUÁRIOS ━━━');
  ...
  const sellerResp = await api(`/admin/tenants/${tenantId}/users`, {
    method: 'POST',
    body: JSON.stringify({ name: 'João Vendedor', email: 'joao.vendedor@dali.com', password: 'vendedor123', role: 'CASHIER', pin: '1234' }),
  });
  if (sellerResp.ok) { created.users.push(sellerResp.data); console.log('✅ Vendedor:', sellerResp.data.user?.email || sellerResp.data.email); }
  else console.log('⚠️  Vendedor:', sellerResp.data.error || '(pode já existir)');

  // 2b. Administrador da loja (OWNER)
  const adminResp = await api(`/admin/tenants/${tenantId}/users`, {
    method: 'POST',
    body: JSON.stringify({ name: 'Maria Admin', email: 'maria.admin@dali.com', password: 'admin123', role: 'OWNER', pin: '5678' }),
  });
  if (adminResp.ok) { created.users.push(adminResp.data); console.log('✅ Admin loja:', adminResp.data.user?.email || adminResp.data.email); }
  else console.log('⚠️  Admin loja:', adminResp.data.error || '(pode já existir)');
  ...
  const sellerLogin = await api('/auth/login', {
    method: 'POST', body: JSON.stringify({ email: 'joao.vendedor@dali.com', password: 'vendedor123' }),
  });
  if (sellerLogin.ok) console.log('✅ Login Vendedor OK:', sellerLogin.data.user.email, '| Tenant:', sellerLogin.data.tenant?.slug);
  else console.log('❌ Login Vendedor falhou:', sellerLogin.data.error);

  const adminLogin = await api('/auth/login', {
    method: 'POST', body: JSON.stringify({ email: 'maria.admin@dali.com', password: 'admin123' }),
  });
  if (adminLogin.ok) console.log('✅ Login Admin loja OK:', adminLogin.data.user.email, '| Tenant:', adminLogin.data.tenant?.slug);
  else console.log('❌ Login Admin loja falhou:', adminLogin.data.error);

  // Forgot password
  const fp = await api('/auth/forgot-password', {
    method: 'POST', body: JSON.stringify({ email: 'joao.vendedor@dali.com' }),
  });
  ...
  const tpl1 = await api('/variation-templates', {
    method: 'POST', body: JSON.stringify({
      name: 'Tamanho (Roupas)',
      dimensions: [{ type: 'TAMANHO_LETRA', label: 'Tamanho', options: ['PP','P','M','G','GG'], orderIndex: 0 }],
    }),
  });
  if (tpl1.ok) { created.templates.push(tpl1.data); console.log('✅', tpl1.data.name); }
  else console.log('⚠️  Tpl:', tpl1.data.error);
  ...
  if (tpl2.ok) { created.templates.push(tpl2.data); console.log('✅', tpl2.data.name); }
  else console.log('⚠️  Tpl:', tpl2.data.error);
  ...
  for (const c of catDefs) {
    const body = { ...c };
    if ((c.name === 'Roupas' || c.name === 'Acessórios') && created.templates[0])
      body.variationTemplateId = created.templates[0].id;
  ...
  for (const s of supDefs) {
    ...
    if (r.ok) { created.suppliers.push(r.data); console.log('✅', r.data.name); }
  ...
  for (const p of simple) {
    ...
    if (r.ok) { created.products.push(r.data); console.log('✅', r.data.name, '| R$', r.data.price); }
    else console.log('⚠️', p.name, r.data.error);
  }

  // 7b. Produto com variações
  if (created.templates[0]) {
    const vp = await api('/products', {
      method: 'POST', body: JSON.stringify({
        name: 'Camiseta Basic',
    ...
    if (vp.ok) { created.products.push(vp.data); console.log('✅', vp.data.name, '|', (vp.data.variations || []).length, 'vars'); }
    else console.log('⚠️  Camiseta:', vp.data.error);
  }
  ...
  if (created.suppliers.length >= 2 && created.products.length >= 3) {
    ...
    if (p1.ok) {
      console.log('✅ Compra #1 (DRAFT):', p1.data.orderNumber);
      const r1 = await api(`/purchases/${p1.data.id}/receive`, { method: 'POST' });
      if (r1.ok) { created.purchases.push(r1.data); console.log('   ✅ Recebida! Lote PEPS criado.'); }
    ...
    if (p2.ok) {
      console.log('✅ Compra #2 (DRAFT):', p2.data.orderNumber);
      const r2 = await api(`/purchases/${p2.data.id}/receive`, { method: 'POST' });
      if (r2.ok) { created.purchases.push(r2.data); console.log('   ✅ Recebida! 2 lotes PEPS criados.'); }
      else console.log('   ⚠️  Receive:', r2.data.error);
    } else console.log('⚠️  Compra #2:', p2.data.error);
  }

  // ====================================================================
  // 9. VENDAS (PEPS consome dos lotes)
  // ====================================================================
  console.log('\n━━━ 9. VENDAS ━━━');
  ...
  if (created.products.length >= 4) {
    ...
    if (o1.ok) { created.orders.push(o1.data); console.log('✅ Venda #1 (PIX 2x Água):', o1.data.orderNumber, '| R$', o1.data.total); }
    else { console.log('⚠️  Venda #1:', o1.data.error); if (o1.data.details) console.log('   Details:', JSON.stringify(o1.data.details)); }

    // Venda 2: Cartão com desconto, múltiplos itens
    ...
    if (o2.ok) { created.orders.push(o2.data); console.log('✅ Venda #2 (Crédito 3 itens):', o2.data.orderNumber, '| R$', o2.data.total); }
    else { console.log('⚠️  Venda #2:', o2.data.error); if (o2.data.details) console.log('   Details:', JSON.stringify(o2.data.details)); }
    ...
    if (o3.ok) { created.orders.push(o3.data); console.log('✅ Venda #3 (Dinheiro 5x Água):', o3.data.orderNumber, '| R$', o3.data.total); }
    else { console.log('⚠️  Venda #3:', o3.data.error); if (o3.data.details) console.log('   Details:', JSON.stringify(o3.data.details)); }

    // Venda 4: Cancelamento de venda
    if (created.orders.length > 0) {
      const firstId = created.orders[0].id;
      const cancel = await api(`/orders/${firstId}/cancel`, { method: 'POST' });
      if (cancel.ok) console.log('✅ Venda #' + created.orders[0].orderNumber + ' cancelada! Estoque revertido.');
      else console.log('⚠️  Cancelamento:', cancel.data.error);
    }
  }
  ...
  if (created.products.length > 1) {
    ...
    const adj1 = await api('/inventory/adjust', {
      method: 'POST', body: JSON.stringify({
        ...
        type: 'ADJUSTMENT_IN',
        quantity: 10,
        reason: 'Reposição de inventário — sobra encontrada',
      }),
    });
    if (adj1.ok) { created.adjustments.push(adj1.data); console.log('✅ Ajuste +10 Arroz (ADJUSTMENT_IN)'); }
    else console.log('⚠️  Ajuste +:', adj1.data.error);
    ...
    const adj2 = await api('/inventory/adjust', {
      method: 'POST', body: JSON.stringify({
        ...
        type: 'ADJUSTMENT_OUT',
        quantity: 5,
        reason: 'Perda por validade vencida',
      }),
    });
    if (adj2.ok) { created.adjustments.push(adj2.data); console.log('✅ Ajuste -5 Água (ADJUSTMENT_OUT)'); }
    else console.log('⚠️  Ajuste -:', adj2.data.error);
  }
  ...
  if (prods.ok) {
    ...
    for (const p of list.slice(0, 10)) {
      const vars = p.variations?.length ? ` [${p.variations.length} vars]` : '';
  ...
  if (cats.ok) console.log(`\n📁 Categorias: ${(cats.data.categories || cats.data).length}`);

  // Suppliers
  const sups = await api('/suppliers');
  if (sups.ok) console.log(`🏭 Fornecedores: ${(sups.data.suppliers || sups.data).length}`);
  ...
  if (purch.ok) {
    ...
    console.log(`\n📥 Compras: ${list.length}`);
    list.forEach(p => console.log(`   #${p.orderNumber} | ${p.status} | ${p.supplier?.name || '?'}`));
  }
  ...
  if (batches.ok) {
    ...
    console.log(`\n📊 Lotes: ${list.length}`);
    list.slice(0, 8).forEach(b => console.log(`   ${b.product?.name || b.productId} | Qtd: ${b.quantity} | Restante: ${b.remainingQty} | Custo: R$ ${b.unitCost}`));
  ...
  if (movs.ok) {
    ...
    list.slice(0, 8).forEach(m => console.log(`   ${m.type} | ${m.product?.name || m.productId} | Qtd: ${m.quantity}`));
    if (list.length > 8) console.log(`   ... mais ${list.length - 8}`);
  }

  // Cash flow
  const cf = await api('/finance/cash-flow');
  if (cf.ok) console.log(`\n💰 Caixa: R$ ${cf.data.totalIn || 0} entradas / R$ ${cf.data.totalOut || 0} saídas`);

  // Today summary
  const today = await api('/orders/today-summary');
  if (today.ok) console.log(`📊 Hoje: ${today.data.totalOrders || 0} vendas, R$ ${today.data.totalRevenue || 0}`);
